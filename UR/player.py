...
class Player():
	...
	def possible_moves(self, board, dice_result):
		pieces = [i for i in self.pieces if i.completed is False]
		possible_moves = []
		for piece in pieces:
			if piece.completed is False and\
				self.check_move_possible(board, piece, dice_result):
	...
	def check_move_possible(self, board, piece, dice_result):
		...
		if piece.square+dice_result > 14:
			return True
		square_id = str(piece.square+dice_result)+self.player if\
			piece.square+dice_result not in list_shared_squares else\
			str(piece.square+dice_result)+'S'
